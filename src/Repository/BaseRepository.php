...
abstract class BaseRepository
{
    ...
    public function getFormAll($orderBy = 'name', $direction = 'ASC')
    {
        $collection = $this->findBy([], [$orderBy => $direction]);
        return $this->makeSimpleForm($collection);
    }

    private function makeSimpleForm(array $collection)
    {
        $ret = [];
        foreach ($collection as $single) {
            $ret[$single->getNameTrans()] = $single;
        }
        return $ret;
    }
}